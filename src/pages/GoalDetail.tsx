...
const GoalDetail = () => {
  ...
  const fetchContributions = async () => {
    try {
      ...
      } = await supabase.from('contributions').select('*').eq('goal_id', id).order('contribution_date', {
        ascending: true
      });
  ...
  if (loading) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4">
  ...
  return <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      {/* Header */}
      <header className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border-b border-gray-200 dark:border-gray-700 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <Button variant="ghost" onClick={() => navigate('/dashboard')} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Go back to dashboard">
            ...
            <div className="flex space-x-3">
              <Button variant="outline" onClick={() => navigate(`/goals/${id}/edit`)} className="border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800">
                <Edit className="mr-2 h-4 w-4" />
                Edit Goal
              </Button>
              <Button onClick={() => setShowFulfillPledge(true)} className="bg-gradient-to-r from-green-600 to-blue-600 hover:from-green-700 hover:to-blue-700 text-white dark:text-white">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-500 text-left">
                      {contributionStarted ? 'Active' : `Starts ${contributionStartDate.toLocaleDateString('en-US', {
                      month: 'short'
                    })}`}
                    </p>
                  </div>
        ...
        <Card className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm hover:shadow-xl transition-all duration-300 border-gray-200 dark:border-gray-700">
          <CardHeader>
            <CardTitle className="text-gray-900 dark:text-white text-left">Financial Progress Chart</CardTitle>
      ...
      <FulfillPledgeDialog open={showFulfillPledge} onOpenChange={setShowFulfillPledge} goalId={goalData.id} goalName={goalData.name} monthlyPledge={goalData.monthly_pledge} goalCreatedAt={goalData.created_at} contributions={contributions} onSuccess={fetchContributions} />
    </div>;
};
